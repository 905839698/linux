...
1.cpu信息
	psutil.cpu_times()	完整cpu信息
	psutil.cpu_times().user 用户cpu时间
	psutil.cpu_times().iowait io等待时间
	psutil.cpu_count() cpu逻辑个数
	psutil.cpu_count(logical=False) cpu物理个数
	psutil.cpu_percent() cpu的使用率

2.内存信息      内存信息的获取主要使用virtual_memory的方法，swap使用的就是swap_memory
svmem(total=4137029632, available=3521974272, percent=14.9,
    used=348778496, free=2162171904, active=795049984, inactive=985239552,
    buffers=86847488, cached=1539231744, shared=2838528, slab=143900672)
    percent表示实际使用的内存占比，available表示可使用的内存

3.硬盘信息      主要两部分 一个是磁盘利用率，一个是io，分别可以通过
    disk_usage和disk_io_counters获取
